...
<html lang="es">
  ...
  <body>
    ...
    <script src="../js/lib/FixedColumns.min.js"></script>
                    ...
                    <?php
                        if ($result = $bd->query("SELECT * FROM users ORDER BY area")) {
                            while ($row = $result->fetch_array(MYSQLI_ASSOC)){
                                echo '<tr><td>'.$row["name"].'</td><td>'.$row["lastname"].'</td><td>'.str_replace(' ', '',$row["cellphone"]).'</td><td>'.$row["aiesecemail"].'</td><td>'.$row["area"].'</td><td>'.$row["local_comitee"].'</td>';
                            }
                            /* free result set */
                            $result->close();
                        }
